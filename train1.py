...
#%%
epochs = 10
num_steps_in_epoch = train_dataset.shape[0]/batch_size #125000
#%%
...
with tf.Session(graph=graph) as session:
  ...
  for epoch in epochs:
    for step in range(num_steps):
      ...
      if (step % 1000 == 0):
        print(str(step) + ' Minibatch loss at step %d: %f' % (step, l))
        print(str(step) + ' Minibatch accuracy: %.1f%%' % accuracy(predictions, batch_labels))
        print(str(step) + ' Validation accuracy: %.1f%%' % accuracy(
          valid_prediction.eval(), valid_labels))
    print("\nEpoch " + str(epoch) + 'Test accuracy: %.1f%%' % accuracy(test_prediction.eval(), test_labels))
